<template>
  <div>
    <el-dialog
      width="30%"
      title="排班"
      :visible.sync="addDoUserDialogFormVisible"
      @close="cancelAdd"
    >
      ...
      <div style="margin-left: 240px">
        <el-button type="primary" @click="confirmCheck" icon="el-icon-edit"
          >提交</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      week1: this.week,
      userid1: this.userid,
      addDoUserDialogFormVisible: true,
      checkList: [],
      today: "",
    };
  },
  ...
  methods: {
    confirmCheck() {
      ...
      //发ajax请求, 包括dutyuserid和checkList数组;
      this.$axios
        .post(
          "/api/arrange/addArr?dutyuserid=" +
            dutyuserid +
            "&checkListStr=" +
            checkListStr +
            "&createuserid=" +
